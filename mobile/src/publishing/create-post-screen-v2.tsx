...
const LAYOUT_COPY = {
  en: { title: 'Post layout', grid: 'Grid', carousel: 'Slides' },
  fr: { title: 'Disposition', grid: 'Grille', carousel: 'Diaporama' },
  ru: { title: 'Вид публикации', grid: 'Сетка', carousel: 'Слайд' },
} as const;

function layoutCopy() {
  return LAYOUT_COPY[appLanguage as keyof typeof LAYOUT_COPY] ?? LAYOUT_COPY.en;
}
...
export default function CreatePostScreenV2() {
  const { request, status } = useAuth();
  const layoutUi = layoutCopy();
  ...
  const publish = async () => {
    ...
    try {
      ...
      Alert.alert(t('postPublished'), t('postPublishedHint'), [{
        text: t('viewPost'),
        onPress: () => router.replace({ pathname: '/post/[postId]', params: { postId: String(post.id) } }),
      }]);
    } catch (caught) {
  ...
  return (
    <Screen contentStyle={styles.screen}>
      <View style={styles.topRow}>
        <View style={styles.headerWrap}><BrandHeader title={t('createPost')} showNotifications={false} /></View>
        <Pressable accessibilityLabel={t('close')} accessibilityRole="button" onPress={close}
          style={({ pressed }) => [styles.close, pressed && styles.pressed]}
        ><Text style={styles.closeText}>×</Text></Pressable>
      </View>

      <View style={styles.composer}>
        <Field label={t('postCaption')} maxLength={5000} multiline onChangeText={setContent}
          placeholder={t('postCaptionPlaceholder')} value={content} style={styles.captionInput} />

        <View style={styles.mediaToolbar}>
          <View style={styles.mediaCopy}>
            <Text style={styles.mediaTitle}>{t('postMedia')}</Text><Text style={styles.mediaHint}>{media.length}/{MAX_MEDIA}</Text>
          </View>
          <Pressable accessibilityRole="button" disabled={media.length >= MAX_MEDIA} onPress={() => void pickMedia()}
            style={({ pressed }) => [styles.addMedia, media.length >= MAX_MEDIA && styles.disabled, pressed && styles.pressed]}
          >
            <Text style={styles.addMediaPlus}>+</Text>
            <Text style={styles.addMediaText}>{media.length ? t('addMoreMedia') : t('choosePostMedia')}</Text>
          </Pressable>
        </View>
            ...
            {media.map((item) => (
              <View key={item.key} style={styles.previewCard}>
                {item.type === 'image'
                  ...
                  : <View style={styles.videoPreview}><Text style={styles.videoIcon}>▶</Text><Text style={styles.videoLabel}>{t('video')}</Text></View>}
                <Pressable accessibilityLabel={t('removeMedia')} accessibilityRole="button"
        ...
        {media.length > 1 ? (
          <View style={styles.layoutSection}>
            <Text style={styles.optionLabel}>{layoutUi.title}</Text>
            <View style={styles.layoutRow}>
              <LayoutChoice icon="▦" label={layoutUi.grid} selected={layout === 'grid'} onPress={() => setLayout('grid')} />
              <LayoutChoice icon="⇆" label={layoutUi.carousel} selected={layout === 'carousel'} onPress={() => setLayout('carousel')} />
            </View>
          </View>
        ) : null}

        <View style={styles.divider} />
        <Field label={t('postLocationOptional')} maxLength={120} onChangeText={setLocation}
          placeholder={t('postLocationPlaceholder')} value={location} />

        <View style={styles.optionSection}>
          <Text style={styles.optionLabel}>{t('postVisibility')}</Text>
          <View style={styles.visibilityRow}>
            {VISIBILITY_OPTIONS.map((option) => (
              <Pressable accessibilityRole="button" accessibilityState={{ selected: visibility === option }} key={option}
                onPress={() => setVisibility(option)}
                style={({ pressed }) => [styles.visibilityButton, visibility === option && styles.visibilitySelected, pressed && styles.pressed]}
              ><Text style={visibility === option ? styles.visibilitySelectedText : styles.visibilityText}>{visibilityLabel(option)}</Text></Pressable>
            ))}
          </View>
        </View>
        ...
        <View style={styles.submitRow}>
          <Button label={t('cancel')} onPress={close} size="compact" variant="ghost" />
          <View style={styles.publishButton}><Button label={t('publishPost')} loading={submitting} onPress={() => void publish()} /></View>
        </View>
      </View>
    </Screen>
  );
...
function LayoutChoice({ icon, label, selected, onPress }: { icon: string; label: string; selected: boolean; onPress: () => void }) {
  return (
    ...
    >
      ...
      <Text style={[styles.layoutText, selected && styles.layoutTextSelected]}>{label}</Text>
    </Pressable>
  );
}
...
const styles = StyleSheet.create({
  ...
  composer: { backgroundColor: '#003c3c', borderColor: 'rgba(4,197,191,.30)', borderWidth: 1, borderRadius: radius.large, padding: spacing.md, gap: spacing.md },
  captionInput: { minHeight: 120, backgroundColor: 'rgba(0,9,17,.42)' },
  mediaToolbar: { minHeight: 42, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: spacing.sm },
  ...
  addMedia: { minHeight: 38, flexDirection: 'row', alignItems: 'center', gap: spacing.xs, paddingHorizontal: spacing.sm, borderRadius: radius.pill, backgroundColor: 'rgba(0,9,17,.32)', borderWidth: 1, borderColor: 'rgba(4,197,191,.28)' },
  ...
  removeButton: { position: 'absolute', top: 5, right: 5, width: 28, height: 28, alignItems: 'center', justifyContent: 'center', borderRadius: 14, backgroundColor: 'rgba(0,10,18,.88)' },
  removeText: { color: colors.text, fontSize: 21, lineHeight: 22 },
  layoutSection: { gap: spacing.xs },
  layoutRow: { flexDirection: 'row', gap: spacing.sm },
  layoutChoice: { flex: 1, minHeight: 56, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: spacing.sm, borderRadius: radius.medium, borderWidth: 1, borderColor: 'rgba(4,197,191,.20)', backgroundColor: 'rgba(0,9,17,.36)' },
  layoutChoiceSelected: { borderColor: colors.primary, backgroundColor: colors.primarySoft },
  layoutIcon: { color: colors.textMuted, fontSize: 21 },
  layoutIconSelected: { color: colors.primary },
  layoutText: { color: colors.textMuted, fontSize: 12, fontWeight: '800' },
  layoutTextSelected: { color: colors.text, fontWeight: '900' },
  divider: { height: 1, backgroundColor: 'rgba(4,197,191,.18)' },
  ...
  visibilityButton: { flex: 1, minHeight: 40, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(4,197,191,.20)', borderRadius: radius.medium, backgroundColor: 'rgba(0,9,17,.36)', paddingHorizontal: spacing.xs },
  ...
  soonChip: { minHeight: 32, justifyContent: 'center', paddingHorizontal: spacing.sm, borderRadius: radius.medium, backgroundColor: 'rgba(4,197,191,.08)', borderWidth: 1, borderColor: 'rgba(4,197,191,.14)' },
