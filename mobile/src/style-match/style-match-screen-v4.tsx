...
export default function StyleMatchScreenV4() {
  ...
  const loadOverview = useCallback(async () => {
    ...
    try {
      ...
      const curatedPreviews = preview.results.slice(0, 3);
      const fallbackPreviews = overview.active_session?.cards.slice(0, 3)
        ?? overview.latest_result?.saved_cards.slice(0, 3)
        ?? [];
      setPreviewCards(curatedPreviews.length ? curatedPreviews : fallbackPreviews);
      setSession(null);
      setMode('intro');
    } catch {
        ...
        {currentCard ? (
          <>
            ...
            <View style={styles.actions}>
              <ActionButton symbol={ACTION_SYMBOLS.reject} label={t('styleMatchReject')} variant="reject" disabled={Boolean(busyAction)} loading={busyAction === 'reject'} onPress={() => void react('reject', 'left')} />
              <ActionButton symbol={ACTION_SYMBOLS.save} label={session.current_saved ? t('styleMatchSaved') : t('styleMatchSave')} variant={session.current_saved ? 'saveActive' : 'save'} disabled={Boolean(busyAction)} loading={busyAction === 'save'} onPress={() => void toggleSaved()} />
              <ActionButton symbol={ACTION_SYMBOLS.like} label={t('styleMatchLike')} variant="like" disabled={Boolean(busyAction)} loading={busyAction === 'like'} onPress={() => void react('like', 'right')} />
              <ActionButton symbol={ACTION_SYMBOLS.favorite} label={t('styleMatchFavorite')} variant="favorite" disabled={Boolean(busyAction)} loading={busyAction === 'favorite'} onPress={() => void react('favorite', 'right')} />
            </View>
          ...
          {[0, 1, 2].map((index) => {
            ...
            return (
              <View key={index} style={[styles.previewCard, index === 0 ? styles.previewCenter : index === 1 ? styles.previewLeft : styles.previewRight]}>
                {preview ? <Image resizeMode="cover" source={{ uri: preview.image_url }} style={styles.previewImage} /> : <View style={styles.previewFallback}><View style={styles.previewSheen} /></View>}
              </View>
            );
          })}
...
function ActionButton({ label, symbol, variant, disabled, loading, onPress }: {
  label: string;
  symbol: string;
  ...
  return (
    ...
    >
      {loading ? (
        ...
        <Text style={[
          styles.actionSymbol,
          isLike && styles.actionSymbolLike,
          activeSave && styles.actionSymbolSaveActive,
        ]}>{symbol}</Text>
      )}
    </Pressable>
  );
...
const styles = StyleSheet.create({
  ...
  actionButtonLike: { width: 64, height: 64, borderRadius: 32, borderColor: '#f24185', backgroundColor: '#d82978' },
  actionButtonSaveActive: { borderColor: colors.primary },
  actionSymbol: { color: '#d9edf0', fontSize: 25, lineHeight: 29, fontWeight: '500', textAlign: 'center', includeFontPadding: false },
  actionSymbolLike: { color: colors.white, fontSize: 30, lineHeight: 33, fontWeight: '400' },
