...
const publishingApi = source('src/publishing/publishing-api.ts');
const publishingBackend = source('../mobile_api/publishing_views.py');
...
check(composer.includes('checked={isAd}') && composer.includes('label={ui.markAsAd}'), 'Create Post exposes the web Mark as Ad control');
check(composer.includes("useState<FeedPost['layout']>('grid')") && composer.includes("['grid', 'carousel'] as FeedPost['layout'][]"), 'Create Post exposes the web Grid / Carousel selector');
check(composer.includes("layout: media.length > 1 ? layout : 'grid'"), 'Single-media publishing safely falls back to grid layout');
check(publishingApi.includes("body.append('is_ad', String(draft.isAd))") && publishingApi.includes("body.append('layout', draft.layout)"), 'Publishing API sends the selected ad flag and media layout');
check(publishingBackend.includes('is_ad = serializers.BooleanField(required=False)') && publishingBackend.includes('is_ad=values.get("is_ad", False)'), 'Mobile backend validates and persists the web ad flag');
check(composer.includes('ui.createPoll') && composer.includes('ui.schedule') && composer.includes('ui.coauthors') && composer.includes('detail={ui.later} disabled'), 'Unimplemented web publishing options stay explicitly Coming soon');
...
check(mediaExport.includes("from './post-media-v2'"), 'Feed activates the v2 media renderer');
...
check(media.includes('<View pointerEvents="none" style={styles.videoThumb}>'), 'Video thumbnail native views cannot steal taps from the parent Pressable');
check(media.includes('<View pointerEvents="none" style={styles.videoThumbOverlay}>'), 'Video thumbnail overlay stays touch-transparent');
check(media.includes('Animated.spring(activation') && media.includes('bounciness: 5'), 'Active thumbnail transitions with a spring animation');
check(media.includes('Animated.timing(transition') && media.includes('Easing.out(Easing.cubic)'), 'Main carousel media uses a smooth cubic transition');
check(media.includes('mainOpacity') && media.includes('mainScale'), 'Main media animates opacity and scale when selection changes');
check(media.includes('const baseSize = largeThumbSet ? 82 : 90') && media.includes('const activeScale = largeThumbSet ? 1.28 : 1.22'), 'Thumbnail geometry uses stable base sizes with animated active scale');
